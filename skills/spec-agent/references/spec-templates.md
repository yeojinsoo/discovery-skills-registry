# Spec Templates — PROBLEM.md + SPEC.md 표준 구조
...
§2가 `[가설] 원인 미상`인 경우, SPEC.md 첫 서브태스크로 삽입:
...
|-----------|--------|---------------|:---------:|:----:|
| §2 원인이 [확인됨]으로 갱신되었다 | PROBLEM.md §2에 `**[확인됨]**` 태그가 존재한다 | — | unit | — |
| 확인된 원인에 대응하는 수정 방향이 기술되었다 | PROBLEM.md §2에 "수정 방향: ..." 텍스트가 존재한다 | — | unit | — |
```
...
|:----:|-------------|--------------|:-----------:|:---------:|----------|:-------:|
| R-1 | {IN 항목} | {S1, S2...} | Strong/Weak | unit/api/browser/visual/db | {Weak 시 보강 방법} | COVERED |
| R-2 | {IN 항목} | {S3} | Strong/Weak | unit/api/browser/visual/db | {Weak 시 보강 방법} | COVERED |
...
> **Exploration Spec 상태 전이 추가 규칙**: Exploration Spec의 DONE 전이 조건은 Acceptance Gate 통과 **+ 후속 실행 스펙 PROBLEM.md 존재**(S2 Oracle로 검증)이다. 후속 실행 스펙이 존재하지 않으면 DONE으로 전이할 수 없다.

---

## Exploration Spec 템플릿 (Wicked Problem 케이스)

spec-agent create G0에서 Wicked Problem으로 분류된 경우 사용하는 템플릿. 일반 SPEC.md와의 차이점:
- Contract 산출물이 **탐색 결과 문서 + 후속 실행 스펙 명세**
- 서브태스크가 **Discover→Define** 2단계로 제한 (구현은 후속 실행 스펙으로 위임)
- Oracle 등급 Weak 허용 (탐색 산출물은 구조만 검증 가능)

```markdown
# {spec-name} — Exploration Spec
...
이 실행 스펙은 {미확정 문제/기술적 불확실성}을 입력으로 받아 **탐색 결과 문서 + 후속 실행 스펙 명세**를 생산한다.
...
| 항목 | IN/OUT | 근거 / 위임처 |
|------|:------:|--------------|
| 대안 탐색 및 비교 분석 | **IN** | 탐색 실행 스펙의 핵심 책임 |
| 최적 접근법 선정 및 문서화 | **IN** | Define 단계에서 확정 |
| 실제 구현 (Design/Deliver) | **OUT** | 후속 실행 스펙으로 위임 |

---

### SPEC-to-TEST 매트릭스

| SPEC IN 항목 | 검증 서브태스크 | Oracle 등급 | Test-Type | 커버리지 |
|-------------|--------------|:-----------:|:---------:|:-------:|
| 대안 탐색 | S1 (Discover) | Weak | unit/api/browser/visual/db | COVERED |
| 접근법 선정 | S2 (Define) | Weak | unit/api/browser/visual/db | COVERED |

> Oracle 등급 Weak 허용 사유: 탐색 산출물의 "품질"은 주관적이므로 구조(존재 여부, 형식)만 기계적 검증. 내용의 적절성은 후속 실행 스펙의 VALIDATE에서 확인.

> **Q1 (과소검증)**: Contract 산출물(탐색 결과 문서, 후속 실행 스펙 명세) 모두 S1/S2의 Predicate로 커버됨
...
| 소스 | 서브태스크 | 격리 방법 | Tier |
|------|----------|----------|:----:|
| (탐색 실행 스펙은 일반적으로 비결정론 소스 없음 — 해당 시 행 추가) | — | — | — |
...
| Predicate | Oracle | Prerequisites | Test-Type | R-ID |
|-----------|--------|---------------|:---------:|:----:|
| 2개 이상의 대안이 탐색 결과 문서에 기술되었다 | 탐색 문서에 `## 대안` 섹션이 존재하고 2개+ 항목이 있다 | — | unit/api/browser/visual/db | — |
| 각 대안에 장단점이 명시되었다 | 각 대안 항목에 `장점:`, `단점:` 또는 동등 구조가 존재한다 | — | unit/api/browser/visual/db | — |

### S2: Define — 접근법 선정 + 후속 실행 스펙 명세 [ ]

**Action**: S1의 대안을 비교 분석하여 최적 접근법을 선정하고, 후속 실행 스펙의 PROBLEM.md 초안을 작성한다.

| Predicate | Oracle | Prerequisites | Test-Type | R-ID |
|-----------|--------|---------------|:---------:|:----:|
| 선정된 접근법과 선정 사유가 문서화되었다 | 탐색 문서에 `## 선정 결과` 섹션이 존재한다 | — | unit/api/browser/visual/db | — |
| 후속 실행 스펙의 PROBLEM.md 초안이 존재한다 | `specs/{후속-spec-name}/PROBLEM.md`가 §1~§3을 포함한다 (초안이므로 §2 [가설] 허용) | — | unit/api/browser/visual/db | — |

> **후속 실행 스펙명 규칙**: `{현재-spec-name}-impl` 접미사를 기본으로 사용한다. 탐색 결과가 여러 구현 실행 스펙으로 분기되면 `-impl-{N}` 접미사를 사용한다. 사용자가 다른 이름을 지정하면 해당 이름을 우선한다.
