...
3. 둘 다 실패 → 안내 출력 후 **즉시 종료** (이후 단계 진행하지 않음):
   ...
   예: /spec-agent status --repo commerceos-backend-v2 --project AG-1614
   ```
...
3. 둘 다 실패 → 안내 출력 후 **즉시 종료**:
   ...
   예: /spec-agent status --project AG-1614
   ```
...
- **미존재** → 한 줄 안내 후 종료:
  ```
  '{project_name}' 프로젝트의 spec-agent 데이터가 없습니다.
  ```
  디렉토리를 생성하지 않는다.
- **존재** → Step 1 진행.
...
- **progress.md** 읽기:
  - `[x]` 개수 = 완료 서브태스크
  - `[x]` + `[ ]` 총합 = 전체 서브태스크 (progress.md에 `[ ]`가 없으면 SPEC.md의 Section 1에서 서브태스크 총 수 카운트)
  - `## 활성 경고` 섹션 내용
  - `## 현재/다음` 섹션 내용
...
history.jsonl에서 최신 `validation_completed`의 `ts`와, 해당 검증 이후의 `spec_updated` 이벤트 존재 여부를 비교.
- `spec_updated.ts > validation_completed.ts`이면 → stale 플래그 설정
...
스펙의 Context Sources에 연결되지 않은 컨텍스트 파일이 있는지 확인 → unmapped_contexts 목록 생성.
...
#### {spec_name} — `{STATUS}` v{버전}

> {Contract Statement 첫 1-2문장 — 이 스펙의 목표}

| 파이프라인 | Context | Spec | Exec | Validate | Summary |
|:----------:|:-------:|:----:|:----:|:--------:|:-------:|
| {spec_name} | {✅/—} | {✅/—} | {✅/🔄/—} | {✅/⚠️/—} | {✅/—} |
...
(스펙이 여러 개면 각각 반복. IN_PROGRESS > PLANNED > DONE 순으로 정렬)
...
{stale이면: "  ⚠️ 검증 이후 스펙이 수정됨 — 재검증 권장"}
...
| 우선순위 | 조건 | 추천 |
|:--------:|------|------|
| 1 | IN_PROGRESS 스펙 존재 | `→ /spec-agent exec {spec_name}` — 실행 이어서 진행 |
| 2 | PLANNED 스펙 존재 | `→ /spec-agent exec {spec_name}` — 실행 시작 |
| 3 | 스펙이 update된 후 재실행되지 않음 (history에 `spec_updated` 이후 `spec_completed` 없음) | `→ /spec-agent exec {spec_name}` — 수정된 스펙 재실행 |
| 4 | DONE 스펙 존재 + 해당 스펙의 검증 없음 | `→ /spec-agent validate {spec_name}` — DoD 검증 |
| 5 | 검증 결과 PARTIAL 또는 NOT_ACHIEVED | `→ /spec-agent update {spec_name}` — 미달 항목 수정 |
| 6 | 검증이 stale (검증 이후 스펙 수정됨) | `→ /spec-agent validate {spec_name}` — 재검증 필요 |
| 7 | 모든 스펙 DONE + 검증 ALL_ACHIEVED + Summary 없음 | `→ /spec-agent summary create` — PR 본문 생성 |
| 8 | Summary 존재 + 워킹 트리에 변경 있음 | `→ 커밋 & PR 생성 (Summary v{N} 활용 가능)` |
| 9 | 스펙의 Context Sources에 연결되지 않은 새 컨텍스트 파일 존재 | `→ /spec-agent update {spec_name}` — 새 컨텍스트 반영 검토 |
| 10 | 컨텍스트만 있고 스펙 없음 | `→ /spec-agent create <spec-name>` — 새 스펙 생성 |
| 11 | 아무 데이터도 없음 (빈 프로젝트) | `→ /spec-agent context <name>` — 문제 탐색 시작 |
