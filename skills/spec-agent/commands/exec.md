...
```
load_spec → validate_spec → check_history
...
### 1. load_spec — 실행 스펙 로드

```
Read: ${SPEC_DIR}/SPEC.md
Read: ${SPEC_DIR}/PROBLEM.md
Read: ${SPEC_DIR}/sessions.jsonl
```
...
1. `type:checkpoint` 레코드로 마지막 완료 서브태스크 확인 → SPEC.md `[x]` 목록과 대조.
...
3. **[Guard-B] Gate Decision 레코드 검증** (이 스텝에서 `session-log-conventions.md §3-4` 참조):
   ...
   if [ $(wc -l < "${SPEC_DIR}/sessions.jsonl") -le 3 ]; then
     g1=$(jq 'select(.type=="gate_decision" and .gate=="G1")' "${SPEC_DIR}/sessions.jsonl")
     g3=$(jq 'select(.type=="gate_decision" and .gate=="G3")' "${SPEC_DIR}/sessions.jsonl")
     g4=$(jq 'select(.type=="gate_decision" and .gate=="G4")' "${SPEC_DIR}/sessions.jsonl")
...
- `${PROGRESS_FILE}`이 이미 존재하고 SPEC.md 상태가 `IN_PROGRESS`: 이전 실행 상태로 간주, 읽기만 수행
- `${PROGRESS_FILE}`이 이미 존재하고 SPEC.md 상태가 `PLANNED` (DONE→PLANNED 재실행): 기존 progress.md 삭제 후 신규 생성 (이전 실행의 "모두 완료" 상태가 오염되지 않도록)
- Write tool로 `${PROGRESS_FILE}` 생성

> **서브태스크 완료 상태 SOT**: (1) SPEC.md `[x]` 체크리스트 = 최종 권위 (2) progress.md = 실행 중 캐시 (3) sessions.jsonl checkpoint = 감사 로그. 불일치 시 SPEC.md 우선.
...
2. `Read(${SPEC_DIR}/SPEC.md, 해당 서브태스크 구간)` — "이번에 할 것"

**방법 2 (progress.md 미존재 시)**: checkpoint 기반 복원
1. `jq -s 'map(select(.type=="checkpoint")) | last' ${SPEC_DIR}/sessions.jsonl`
...
이전 서브태스크에서 Worker가 변경한 파일 경로 누적 목록을 progress.md의 완료 서브태스크에서 추출하여, Worker 프롬프트의 `관련 파일 맵`으로 주입한다. 첫 번째 서브태스크에서는 SPEC.md의 Action에 언급된 파일 경로 또는 `ls` 기반 디렉토리 구조 요약(주요 디렉토리 1~2 depth)을 제공한다.
...
- AskUserQuestion: "[1] 진행 [2] /spec-agent update 호출 [3] 중단"
- [2] 선택 시 → Skill 도구로 `/spec-agent update {spec_name} --project {project_name} --repo {repo_slug}` 호출
...
- **L1 회귀 검사** (선행 단계 — `rules/regression-test-strategy.md` §2 참조):
  ...
  - 각 PASS 서브태스크의 Oracle을 SPEC.md Predicate/Oracle 테이블에서 추출하여 재실행
...
```jsonl
{"id":"K-{NNN}","spec":"{spec-name}","round":{N},"ts":"{ISO}","cat":"{cat}","confidence":"observed","insight":"{교훈}","tags":[...]}
```
...
- blocker 존재 → **실패 유형 3분류** (`references/test-verification-guide.md` §4 참조)로 원인을 진단한 후 유형별 후속 액션 분기:
  ...
  - **스펙 실패**: Predicate 자체가 달성 불가능 → AskUserQuestion: "[1] /spec-agent update로 스펙 수정 [2] 건너뛰기"
...
- 5회 초과 → sessions.jsonl append + AskUserQuestion:
  "[1] 수동 수정 후 재검증 [2] /spec-agent update {spec_name} --project {project_name} --repo {repo_slug} [3] 건너뛰고 다음 서브태스크"
...
Acceptance Gate 통과 후 DONE 전환 직전에 필수 수행. 전체 실행 스펙 실행 결과를 서로 다른 관점의 에이전트팀이 홀리스틱하게 검토하여, DONE 선언의 정당성을 검증한다.
...
| **Z** | Regression & Risk Assessor | 변경의 회귀 영향과 미검증 영역을 분석. Scope Boundary의 **IN (Invariant)** 항목을 우선 회귀 검증 대상으로 식별. Invariant 위반 시 severity = blocker. 미검증 영역 분석은 §8-1a 가이드라인 참조 | SPEC.md Scope (Invariant 항목 포함), 코드 변경 diff, 관련 모듈 | 회귀 리스크 판정 + Invariant 보존 검증 + 미검증 영역 목록 |
...
설정 파일만 변경하는 스펙 등 해당 없는 항목은 "해당 없음"으로 생략한다.
...
> severity 체계는 서브태스크 Verifier와 CRG에서 동일하다 (blocker/warning/note). 스코프만 다르다 — Verifier는 단일 서브태스크, CRG는 전체 스펙.
...
- [3] "수정 필요 (전체)" → NEEDS_FIX와 동일 수정 사이클 (스펙 재설계 + 재실행)
...
| **NEEDS_REVIEW 경유 순환 가드** | NEEDS_REVIEW→IN_PROGRESS 전환 후 exec 재진입 시, sessions.jsonl에서 이전 `crg_needs_review` 레코드를 확인. 동일 스펙에서 **통산 NEEDS_REVIEW 2회** 도달 시 자동 재실행 선택지를 제거하고 "실행 스펙 재설계 (/spec-agent create) 또는 ABANDONED" 안내 |
...
```bash
result=$(jq -r 'select(.type=="crg" and (.outcome=="crg_pass" or .outcome=="conditional_pass"))' ${SPEC_DIR}/sessions.jsonl)
...
```jsonl
{"type":"spec_completed","name":"{spec_name}","ts":"{ISO 8601}","status":"DONE"}
```
...
스펙 수 판정: `ls "${SPECS_DIR}" | wc -l`

**[A] 단일 스펙 프로젝트 (1개)**:

AskUserQuestion:
  "실행 스펙 `{spec_name}` 실행이 완료되었습니다. 다음 단계를 선택하세요:"
  [1] PR 본문 생성 (`/spec-agent summary create`) (Recommended)
  [2] DoD 검증 (`/spec-agent validate`)
  [3] 나중에

[1] 선택 시 → Skill 도구로 `/spec-agent summary create --project {project_name} --repo {repo_slug}` 호출
[2] 선택 시 → Skill 도구로 `/spec-agent validate --project {project_name} --repo {repo_slug}` 호출

**[B] 다중 스펙 프로젝트 (2개+)**:

AskUserQuestion:
  "실행 스펙 `{spec_name}` 실행이 완료되었습니다.
   이 프로젝트에 {N}개 스펙이 있습니다. 다음 단계를 선택하세요:"
  [1] DoD 검증 (`/spec-agent validate`) (Recommended)
  [2] PR 본문 생성 (`/spec-agent summary create`)
  [3] 나중에

[1] 선택 시 → Skill 도구로 `/spec-agent validate --project {project_name} --repo {repo_slug}` 호출
[2] 선택 시 → Skill 도구로 `/spec-agent summary create --project {project_name} --repo {repo_slug}` 호출
