...
실행 스펙 수정 워크플로우를 실행한다.

### SOT 참조

- `${CLAUDE_SKILL_DIR}/rules/spec-test-alignment.md` — SPEC-TEST 정합성 (R-006)
- `${CLAUDE_SKILL_DIR}/rules/spec-quality-characteristics.md` — 품질 특성 (R-001)
...
```
load_spec → validate_state → context_delta_check
  → identify_change → cascade_analysis
  → GATE-CHANGE-CONFIRM(AskUserQuestion)
  → apply_changes → re_verify → update_artifacts → report
  → GATE-AUTO-EXEC → [YES] invoke /spec-agent exec {spec-name}
```

### Drift-triggered Entry

spec-agent-exec의 drift_check 또는 stagnation 감지에서 호출된 경우:
...
- **SPEC.md / PROBLEM.md**: Edit 도구로 수정
...
Read: ${KNOW_FILE}
# CRG 비평 파일 (존재 시)
ls ${SPEC_DIR}/crg-*.md 2>/dev/null → 존재하면 최신 파일 Read
```
...
| DONE | 수정 불가. 새 실행 스펙 생성 안내 또는 사용자 명시 요청 시 IN_PROGRESS 전환 |
...
SPEC.md `## Context Sources`의 `[ ]` 항목(미반영 파일)을 식별:
...
update 완료 시 반영된 파일을 SPEC.md `## Context Sources`에서 `[x]`로 업데이트.

### 3. identify_change

사용자 요청을 실행 스펙 구성 요소에 매핑. Contract/Predicate 변경 시 안티패턴 탐지.

**CRG-triggered Entry**: `${SPEC_DIR}/crg-*.md` 파일이 존재하면, 최신 crg-{N}.md를 읽어 blocker/warning 항목을 변경 드라이버로 자동 매핑한다.
...
**Predicate/Oracle 수정 시**: Prerequisites 열을 보존한다. Oracle 변경 시 Prerequisites도 재검토한다 (Safe/Stateful 등급 변경 가능성). **레거시 SPEC.md(Prerequisites 열 미존재)**: apply_changes 시 Prerequisites 열을 추가하고, 기존 서브태스크의 값은 모두 "—"(미지정)으로 채운다.

**SPEC-to-TEST 매트릭스 수정 시**: 보강 경로 열을 보존한다. Oracle 등급 변경(Strong↔Weak) 시 보강 경로를 추가/제거한다.

### 6-A. 구조 검증 (기계적)

apply_changes 완료 후, SPEC.md 필수 구조를 기계적으로 검증한다 (create §9-A와 동일):
...
`${HIST_FILE}`에 append:
```jsonl
{"type":"spec_updated","name":"{spec_name}","ts":"{ISO 8601}","changes":"{변경 요약}"}
```
...
- [1] 바로 재실행 → Skill 도구로 `/spec-agent exec {spec_name} --project {project_name} --repo {repo_slug}` 호출
- [2] 나중에 실행

apply_changes에서 적용된 변경이 0건(사용자가 GATE-CHANGE-CONFIRM에서 모든 변경을 거부 등)이면 GATE를 발동하지 않고 report에서 종료한다.

**exec↔update 순환 상한**: exec→update→exec 전환은 **최대 3회**. sessions.jsonl의 연속 `aborted` 레코드로 카운트. 3회 도달 시 자동 재실행 선택지를 제거하고 "[1] 나중에 실행 [2] 실행 스펙 재설계 (/spec-agent create)" 제시.
