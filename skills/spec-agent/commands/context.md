## §CONTEXT — 실행 스펙 컨텍스트 구체화

`spec-agent create` 전/후 언제든 문제·원인·해결 조건·설계 방향·테스트 시나리오를 자유롭게 탐색하고 구체화한다.
컨텍스트는 **세션별 별도 파일**로 축적되며, `spec-agent create` / `spec-agent update` 시 어떤 파일이 아직 반영되지 않았는지 추적된다.
...
각 `spec-agent context` 실행은 **하나의 새 파일**을 생성한다. 기존 파일을 수정하지 않는다.
...
```
parse_args → resolve_next_filename → check_spec_state
  → [seed 있음?]
...
spec_name은 받지 않는다 (context는 프로젝트 레벨).
...
> SKILL.md 공유 유틸리티 `check_spec_state / load_existing_context` 참조.

### 3-1. seed 분기

check_spec_state 이후 seed_file 존재 여부에 따라 분기:
...
2. **치명적 갭 필수**: 치명적 심각도의 갭(§3 전체 부재)은 건너뛸 수 없음. 사용자가 "건너뛰기"를 시도하면 한 번 더 요청 (최대 재요청 2회). 2회 거부 시:
   - 경고 메시지 출력: "⚠️ §3(해결 조건)이 미작성 상태입니다. 이 context로 create를 실행하면 불완전한 실행 스펙이 생성될 수 있습니다."
