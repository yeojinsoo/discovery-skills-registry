# Spec Quality Characteristics — 결정론적 실행 계획의 형식적 특성
...
실행 계획은 **절차서가 아니라 검증 명세**다. "무엇을 해야 하는가"가 아니라 "어떤 상태가 참이어야 하는가"를 선언한다. 이를 통해 실행 스펙의 완료 여부가 **이진적(true/false)**으로 판정되며, 실행자(사람이든 Agent든)가 동일한 입력에 대해 동일한 결과를 재현할 수 있다.
