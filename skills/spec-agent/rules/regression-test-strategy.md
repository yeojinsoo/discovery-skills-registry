...
> **적용 범위**: 실행 스펙 실행 전 과정에서 회귀를 3개 계층으로 감지
...
2. 각 PASS 서브태스크의 Oracle을 SPEC.md Predicate/Oracle 테이블에서 추출
...
> **Oracle 추출**: sessions.jsonl에는 Oracle 명령이 저장되지 않는다. 이전 PASS 서브태스크의 S-ID를 sessions.jsonl에서 확인한 후, SPEC.md의 해당 서브태스크 Predicate/Oracle 테이블에서 Oracle 명령을 읽어 재실행한다.
...
> **Invariant**: CRG의 3-에이전트 구조(X Code Verifier / Y Spec Integrity Critic / Z Regression & Risk Assessor)를 변경하지 않는다. L2는 Agent Z의 기존 역할 내에서 수행한다.

### 3-3. 실행 절차

```
1. SPEC.md Scope Boundary에서 IN (Invariant) 항목을 추출
...
| 입력 | sessions.jsonl + Oracle | SPEC.md Scope + diff | knowledge.jsonl |
...
이 전략은 spec-agent의 모든 실행 흐름에 적용된다:
...
이 규칙 제정 전에 실행 완료된 실행 스펙에는 소급 적용하지 않는다.
