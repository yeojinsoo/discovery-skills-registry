...
> 플랜 실행 산출물의 실행 기반 검증(test 커맨드)에서 사용하는 판정 체계, 브라우저 테스트 패턴, 복합 판정 전략, 실패 분류, 집계 알고리즘, 선택자 전략을 정의한다.
...
1. **Scope IN 확인**: 해당 Oracle이 검증하는 대상이 PLAN.md Scope Boundary의 IN 항목에 속하는가
