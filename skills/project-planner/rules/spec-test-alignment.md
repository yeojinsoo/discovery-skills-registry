# SPEC-TEST Alignment — 프로젝트 플랜 계약 경계 + 검증 정합성

> **용도**: 프로젝트 플랜 생성 전 SPEC/TEST 정합성을 강제하는 게이트 규칙

---

## 0. 핵심 원리

프로젝트 플랜의 완료 조건(DoD)은 SPEC이 선언한 계약 범위와 **정확히 일치**해야 한다.

- SPEC이 더 넓으면 → **미달 완료** (false accept). TEST가 PASS해도 SPEC 미이행.
- TEST가 더 넓으면 → **범위 초과** (scope creep). 불필요한 서브태스크가 나중에 DELEGATED/SKIP 처리.

이 규칙은 `project-plan-quality-characteristics.md`의 F1~F6, O1~O6과 병행 적용된다.
...
2. PLAN.md에 원인 조사 서브태스크(VALIDATE 단계)를 자동 삽입
...
분기:
- **[1] 탐색 프로젝트 플랜 (Recommended)**: 탐색 Contract로 진행 → G1에서 탐색형 Contract Statement 작성
- **[2] 일반 프로젝트 플랜 강제 진행**: §3을 최대한 구체화한 후 일반 Gate 순서대로 진행
...
본 시스템에서는 **프로젝트 플랜 작성 에이전트가 자기 채점**하므로, 자기 확증 편향에 의해 실제보다 낮게(명확하다고) 산출될 가능성이 있다. 따라서:
...
- project-planner create로 G0을 통과한 프로젝트 플랜이 3~5건 누적되면, 사용자가 threshold 적합성을 재검토한다. 재검토 결과는 `${KNOW_FILE}`에 `cat: "decision"` 레코드로 기록한다 (스키마: `${CLAUDE_SKILL_DIR}/rules/knowledge-conventions.md` 참조)
- Threshold 변경 시 이 섹션과 SKILL.md Step 3의 매트릭스를 함께 갱신한다

#### 경계 구간 운용 가이드라인

프로젝트 플랜 작성 에이전트가 자기 채점하므로, 경계 구간(0.15~0.25)에서는 **보수적으로 CLARIFY 판정을 권고**한다. 이는 매트릭스의 `≤ 0.2 → AUTO_PASS` 판정을 무효화하는 것이 아니라, 자기 채점 시 경계 구간에서 의심되면 CLARIFY 쪽으로 판단하라는 운용 가이드라인이다.
...
모든 프로젝트 플랜의 Section 0에 다음 3개 요소를 포함한다.

### 1-A. Contract Statement (계약 문장)

1문장으로 "이 프로젝트 플랜이 생산하는 것"을 명시한다.

**형식**:

```
이 플랜은 {입력}을 받아 {출력}을 생산한다.
```
...
이 프로젝트 플랜이 수행하는 것(IN)과 수행하지 않는 것(OUT)을 명시적으로 구분한다.

| 항목 | IN/OUT | 근거 |
|------|:------:|------|
| (이 프로젝트 플랜의 책임) | **IN** | (왜 이 프로젝트 플랜의 범위인가) |
| (이 프로젝트 플랜의 책임 아님) | **OUT** | (위임처: 플랜 이름/시스템/미정) |

**규칙**:

- IN 1개 이상, OUT 1개 이상 필수
- OUT이 0개인 프로젝트 플랜 → 경계 미설정 경고. 모든 프로젝트 플랜에는 "하지 않는 것"이 있다
- OUT 항목에 **구체적 위임처** 명시 (플랜 이름, 시스템 이름, 또는 "미정 — HITL 결정 필요")
...
| 유형 | 판단 기준 |
|------|----------|
| 런타임 행위 검증 | 이 프로젝트 플랜이 산출물을 만드는가, 산출물이 동작하는지 확인하는가? |
...
- IN 항목 중 `COVERED`가 아닌 것이 있으면 → **플랜 작성 BLOCKED** (서브태스크 추가 또는 SPEC 축소)
...
프로젝트 플랜 작성 완료 후, 아래 3가지 질문으로 정합성을 검증한다.
...
## 4. HITL Gate — 프로젝트 플랜 생성 전 필수 체크리스트

(SOT: commands/create.md G0~G4)

PLAN.md 작성 전에 아래 항목에 모두 응답해야 한다. 미응답 항목이 있으면 프로젝트 플랜 생성 **BLOCKED**.
