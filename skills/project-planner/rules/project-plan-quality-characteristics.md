# Project Plan Quality Characteristics — 결정론적 실행 계획의 형식적 특성

> **용도**: 프로젝트 플랜 작성 시 준수할 품질 특성 가이드

---

## 0. 핵심 원리

실행 계획은 **절차서가 아니라 검증 명세**다. "무엇을 해야 하는가"가 아니라 "어떤 상태가 참이어야 하는가"를 선언한다. 이를 통해 프로젝트 플랜의 완료 여부가 **이진적(true/false)**으로 판정되며, 실행자(사람이든 Agent든)가 동일한 입력에 대해 동일한 결과를 재현할 수 있다.
...
**정의**: 전체 프로젝트 플랜의 완료 조건을 단일 불린 표현식으로 환원한다.
...
- 프로세스 exit code — 환경 전제조건(플랜 Section 0)이 보장하면 결정적
...
| **검토 oracle** | HITL 검토 기준표 | 프로젝트 플랜 문서에 고정 |
...
**메커니즘**: 프로젝트 플랜 Section 0(설계 원칙)에 명시된 원칙 각각이 하나 이상의 서브태스크에 대응되어야 한다. 서브태스크의 Action 행에 "적용 원칙: D-3, AX-7" 등으로 명시하거나, 별도 매핑 테이블로 관리한다.

| 원칙 | 서브태스크 | Predicate |
|------|----------|-----------|
| (설계 원칙 ID) | (서브태스크 ID) | (검증할 Predicate 요약) |

- 원칙에 대응하는 서브태스크가 없으면 해당 원칙이 프로젝트 플랜에 불필요하거나, 서브태스크 누락
...
[ ] Section 0: 설계 원칙 (해당 플랜에 적용되는 원칙 3개 이상, 각 원칙의 Testability 판정)
