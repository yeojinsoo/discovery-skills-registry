# Critical Review Gate (CRG) — 프로젝트 플랜 완료 전 다관점 검토 프로토콜
...
실행 프로토콜(R-004)은 **서브태스크 단위**의 국소 검증이다. 서브태스크별로 Predicate/Oracle이 충족되어도, 전체 프로젝트 플랜 관점에서 다음 문제가 잔존할 수 있다:

- Oracle이 실제 검증 방법과 불일치 (문서상 Strong이나 실제 Weak)
- Scope Boundary 외부로 영향이 누출
- 코드 수정 자체는 올바르나 PLAN.md 명세와 괴리

CRG는 **전체 프로젝트 플랜 실행 결과**를 서로 다른 관점의 에이전트팀이 홀리스틱하게 검토하여, DONE 선언의 정당성을 검증한다.
...
| **Z** | Regression & Risk Assessor | 변경의 회귀 영향과 미검증 영역을 분석. **Invariant 보존 검증 절차** (`regression-test-strategy.md` §3 참조): ① PLAN.md Scope Boundary IN(Invariant) 항목 추출 → ② 코드 변경 diff와 대조 → ③ 위반 시 severity = blocker. 미검증 영역 분석은 `commands/exec.md` §8-1a 가이드라인 참조 | PLAN.md Scope (Invariant 항목 포함), 코드 변경 diff, 관련 모듈 | 회귀 리스크 판정 + Invariant 보존 검증 + 미검증 영역 목록 |
...
- 코드 변경이 PLAN.md 서브태스크 Action과 불일치
...
3. "수정 필요 (전체)" → NEEDS_FIX와 동일한 수정 사이클 진행 (플랜 재설계 + 재실행)
...
**순환 가드**: 동일 플랜에서 NEEDS_REVIEW **통산 2회** 도달 시, IN_PROGRESS 전환을 차단한다. sessions.jsonl에서 `crg_needs_review` outcome 레코드 수로 카운트. 2회 도달 시 선택지: "[1] 프로젝트 플랜 재설계 (/project-planner create) [2] ABANDONED".

### 5-3. 허용/차단 행동

| 행동 | 허용 | 근거 |
|------|:----:|------|
| sessions.jsonl / PLAN.md 읽기 | O | 원인 분석용 |
...
**관리자 선택지**:
1. PLAN.md Contract/Scope 재설계 후 IN_PROGRESS 복귀
2. 현재 범위 축소 후 재승인
3. ABANDONED 결정
```
...
CRG 실행 시 비평 내용을 플랜 디렉토리에 저장한다:

```
repos/{repo_slug}/projects/{project_name}/plans/{plan_name}/
├── PROBLEM.md
├── PLAN.md
├── sessions.jsonl
...
> CRG가 발생하지 않는 플랜에는 `crg-*.md` 파일이 없다.
...
```bash
# CRG 실행 횟수
jq 'select(.type == "crg")' plans/{plan}/sessions.jsonl | jq -s 'length'

# CRG 실패 이력
jq 'select(.type == "crg" and .outcome == "crg_fail")' plans/{plan}/sessions.jsonl
```
...
Acceptance Gate가 정의된 모든 프로젝트 플랜에 CRG를 적용한다.
...
이 규칙 제정 전에 DONE 처리된 프로젝트 플랜에는 소급 적용하지 않는다. 해당 프로젝트 플랜의 구현체에서 문제가 발견되면 신규 프로젝트 플랜을 생성하여 CRG를 적용한다.
