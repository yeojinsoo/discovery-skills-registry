...
| **Z** | Regression & Risk Assessor | 변경의 회귀 영향과 미검증 영역을 분석. Scope Boundary의 **IN (Invariant)** 항목을 우선 회귀 검증 대상으로 식별. Invariant 위반 시 severity = blocker. 미검증 영역 분석은 `commands/exec.md` §8-1a 가이드라인 참조 | PLAN.md Scope (Invariant 항목 포함), 코드 변경 diff, 관련 모듈 | 회귀 리스크 판정 + Invariant 보존 검증 + 미검증 영역 목록 |
